...
static rmt_led_strip_encoder_t *as_led_strip_encoder(rmt_encoder_t *encoder)
{
    return (rmt_led_strip_encoder_t *)encoder;
}
...
{
    if (!encoder || !ret_state) {
        return 0;
    }

    rmt_led_strip_encoder_t *led_encoder = as_led_strip_encoder(encoder);
    if (!led_encoder->bytes_encoder || !led_encoder->copy_encoder) {
